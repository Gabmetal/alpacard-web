...
export default function ScannerClient() {
    ...
    const [scanning, setScanning] = useState(!urlToken);
    const [processing, setProcessing] = useState(false);

    // Effect to handle URL token immediately on load
    useEffect(() => {
        if (urlToken && !result && !processing) {
            handleScan([{ rawValue: urlToken }]);
        }
    }, [urlToken]);
    ...
    const handleScan = async (uploadedFiles: unknown[]) => {
        ...
        try {
            ...
            if (!res.success && res.message === "Unauthorized") {
                // Redirect to sign in, preserving the current URL as redirect_url
            ...
            // Clear the token from URL so refreshing doesn't re-trigger immediately if desired,
            // or leave it. Usually better to clear or replace history to avoid double-submit on refresh.
            if (urlToken) {
